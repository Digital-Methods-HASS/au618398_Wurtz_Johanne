...
head(cities_sf3035) # I use head() and the crs has changed into 3035 and it therefore 2D
```


### Question 1: 
*What are the measurement units of the `cities_sf3035` object?*
It is meters.`sf` will automatically use meters, the units of the CRS.
...
# download.file("https://sciencedata.dk/public/b6b6afdb969d378b70929e86e58ad975/EDH_subset_2021-02-15.json", "inscriptions.json") # The json file is in the W3 HW folder
...
head(inscriptions) #check the first couple of rows
colnames(inscriptions) #check column names
class(inscriptions$coordinates) # check the class of the coordinates - it's a list
...
i_sm <- inscriptions %>%
  ...
  mutate(latitude = latitude,
         longitude = longitude) # The coordinates needs to be numeric so I added as.numeric

# Cleaning up the two new variables
i_sm$latitude <- gsub(")", "", i_sm$latitude) # When using gsub, you replace the first item in the parenthesis with the second item. [c(]) is replaces with 'nothing' - which means it's removed.   
...
i <- inscriptions %>% 
  ...
  filter(coordinates != "list()") %>% 
  # Separating the coordinates column
  separate(col = coordinates, into = c("longitude","latitude"), sep = ",") %>%
  # Making columns for latitude and longitude
  mutate(latitude = latitude, longitude = longitude)

# Just as we did above, we need to remove the "c(" from the longitude and latitude values and making them numeric
...
# I got an error when I tried to filter with both values in one, so I split them into two.
i <- i %>% 
...
cities_it <- st_intersection(insc_ch, st_transform(cities_5km, crs = 3035))
...
```
 ...
 There are 6.5 times more inscriptions in the urban areas compared to the rural areas in Italy.
